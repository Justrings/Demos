...
(function () {
	...
	var downLoadDirectory = function downLoadDirectory() {
		return new Promise(function (resolve, reject) {
			$.ajax({
				...
				success: function success(sourceData) {
	...
	var addDataToHtml = function addDataToHtml(result) {
	...
	var downLoadContent = function downLoadContent(start, end) {
		var data = { key: 'd20700c02fcb0f7e044254e3401b350a',
			catalog_id: "中国文学",
			pn: start,
			rn: end || 10
		};
	...
	var loadChapterFrom = function loadChapterFrom(m, n) {
		//假定的章节范围
		if (m < 0 || n > 300) {
			return;
		}
	...
	var loadChapterClearHtml = function loadChapterClearHtml(m, n) {
		//假定的章节范围
		if (m < 0 || n > 300) {
			return;
		}
	...
	var mountEvent = function mountEvent() {
		//header部分事件   返回 + 详细信息
		...
		//加载下一章
		var loadNextChapter = function loadNextChapter() {
		...
		//加载上一章
		var loadPreviousChapter = function loadPreviousChapter() {
		...
		//章节选择部分 事件
		var changeChapter = function changeChapter(target) {
		...
		var chapterContainer = function chapterContainer() {
			$chapter_control.on('click', '.chapter', function (e) {
				changeChapter(e.target);
			});
		};
		//字体大小调整
		var increaseFontSize = function increaseFontSize() {
			var num = parseInt($article.css("font-size"));
			if (num < 20) {
		...
		//字体颜色切换
		var switchToColor = function switchToColor(target) {
			var length = $bg_colors.length;
			for (var i = 0; i < length; i++) {
		...
		//字体切换触发事件		
		var fontContainer = function fontContainer() {
			$font_container.on('click', '.font-item', function (e) {
				var target = e.target;
		...
		//白天夜晚切换
		var timeSwitch = function timeSwitch() {
		...
		//底部主选项事件触发
		var footerItemContainer = function footerItemContainer() {
			$container.on('click', '.footer-item', function (e) {
				var target = e.target;
				console.log("e.target", e.target);
				switch (true) {
		...
		var headerContainer = function headerContainer() {
			$header.on('click', '.item', function (e) {
				var target = e.target;
				console.log("e.target", e.target);
				switch (true) {
